...
namespace MVCPresentation.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    if (usrMgr.FindUser(model.Email))
                    {
                        ...
                        if (result.Succeeded)
                        {
                            // use the oldUser.roles list to add the internally assignedroles to the user
        ...
        // GET: /Account/RegisterExistingUser
        [Authorize(Roles = "Admin")]
        public ActionResult RegisterExistingUser()
        {
            return View();
        }

        // POST: /Account/RegisterExistingUser
        [HttpPost]
        [Authorize(Roles = "Admin")]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterExistingUser(RegisterExistingUserViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    else // not an existing user, create a DataObjects.User without roles
                    {
                        ...
                        if(usrMgr.AddUser(model.FamilyName, model.GivenName, model.Email) == 1)
                        {
                            var userID = usrMgr.RetrieveUserIDFromUserName(model.Email);
                            var identityUser = new ApplicationUser // if it worked, create an Identity user 
                            {
                                UserID = userID,
                                GivenName = model.GivenName,
                                FamilyName = model.FamilyName,
